...
inline void standardBoxBlur(uint8_t* ioBuffer, uint8_t* tempBuffer, unsigned kernelSizeX, unsigned kernelSizeY, int stride, uint paintSizeWidth, uint paintSizeHeight, bool isAlphaImage, EdgeModeType edgeMode)
{
    int dxLeft = 0;
    int dxRight = 0;
    int dyLeft = 0;
    ...
    for (int i = 0; i < 3; ++i) {
        if (kernelSizeX) {
            ...
            boxBlur(fromBuffer, toBuffer, kernelSizeX, dxLeft, dxRight, 4, stride, paintSizeWidth, paintSizeHeight, isAlphaImage, edgeMode);
        ...
        if (kernelSizeY) {
            ...
            boxBlur(fromBuffer, toBuffer, kernelSizeY, dyLeft, dyRight, stride, 4, paintSizeHeight, paintSizeWidth, isAlphaImage, edgeMode);
            std::swap(reinterpret_cast<std::tuple<uint8_t*> &>(fromBuffer),
                      reinterpret_cast<std::tuple<uint8_t*> &>(toBuffer));
        }
    }
...
void GaussianBlur::platformApplyGeneric(uint8_t* ioBuffer, uint8_t* tmpPixelArray, uint paintSizeWidth, uint paintSizeHeight)
{
    ...
    standardBoxBlur(ioBuffer, tmpPixelArray, kernelSizeX, kernelSizeY, stride, paintSizeWidth, paintSizeHeight, false, m_edgeMode);
}
